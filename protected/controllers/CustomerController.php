...
class CustomerController extends Controller
{
  ...
  public function accessRules()
  {
    return array(
      ...
      'actions' => array('staffadmin', 'admin', 'delete', 'create', 'update', 'stafflist'),
  ...
  public function actionCreate()
  {
    ...
    if (isset($_POST['Customer'])){
      ...
      if ($model->save()) $this->redirect(array('view', 'id' => $model->customer_id));
    }
  ...
  {
    $data = CustomerStaff::model()->findAll('customer_id=:customer_id', array(
      ':customer_id' => (int)$_POST['customer_id']));
    $data = CHtml::listData($data, 'staff_id', 'name');
    echo CHtml::tag('option', array('value' => ""), CHtml::encode("Seçiniz"), true);
    foreach($data as $value => $name){
      echo CHtml::tag('option', array('value' => $value), CHtml::encode($name), true);
    }
  }

  public function ActionStaffAdmin()
  {
    $model = new CustomerStaff('search');
    $model->unsetAttributes(); // clear any default values
    if (isset($_GET['CustomerStaff'])) $model->attributes = $_GET['Customer'];
    $this->render('staffadmin', array('model' => $model));
  }
