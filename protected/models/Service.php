...
class Service extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('customer_id, product_id, member_id', 'length', 'max'=>10),
			array('service_status', 'length', 'max'=>8),
			array('process, explanation', 'safe'),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('service_id, customer_id, product_id, member_id, process, explanation, service_status', 'safe', 'on'=>'search'),
		);
	}
